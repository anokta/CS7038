...
public class HandyDetector : MonoBehaviour
{
    ...
    public delegate Collider2D[] CollisionFunction2D(
		Vector2 position,int mask,float minDepth,float maxDepth);
    ...
    void Start()
    {
        ...
        slideState.Hold += delegate {
            if (slideState.time > flingTime) {
                FlingArgs sArgs = new FlingArgs(Gesture.Press, FlingArgs.State.Fail, slideState.startPos, _device.pos1);
                IFling f = _target as IFling;
                if (f != null) {
                    f.OnGestureFling(sArgs);
                }
        ...
        slideState.Interrupt += delegate {
            if (slideState.time <= flingTime) {
                FlingArgs sArgs = new FlingArgs(_prevType, FlingArgs.State.Fail, slideState.startPos, _device.pos1);
                IFling f = _target as IFling;
                if (f != null) {
                    f.OnGestureFling(sArgs);
                }
        ...
        longState.Interrupt += delegate {
            ILongPress lpress = _target as ILongPress;
            if (lpress != null) {
                LongPressArgs args = new LongPressArgs(Gesture.Press, LongPressArgs.State.Fail, longState.startPos);
                lpress.OnGestureLongPress(args);
            }
        ...
        panState.Interrupt += delegate {
            IPan pan = _target as IPan;
            if (pan != null) {
                PanArgs pArgs = new PanArgs(Gesture.Fling, PanArgs.State.Fail, panState.startPos, _device.pos1, Vector2.zero);
                pan.OnGesturePan(pArgs);
            }
    ...
    static int Supported<T>(List<Transform> list)
    {
        for (int i = 0; i < list.Count; ++i) {
            if (list[i].GetComponent(typeof(T)) != null) {
                return i;
            }
        }
        return -1;
    ...
    List<Transform> GetAllObjects2D(Vector2 pos)
    {
        ...
        list.AddRange(
            from _ in results
               where _.transform != interceptor.transform && _.transform != defaultObject.transform
    ...
    Collider2D[] CustomResults2D(Vector2 position, int mask, float min, float max)
    {
        if (CustomFunction2D != null) {
            return CustomFunction2D.GetResults(position, mask, min, max);
        }
        return null;
    }

    Collider2D[] Raycast2D(Vector2 position, int mask, float min, float max)
    {
        return Physics2D.OverlapPointAll(position, mask, min, max);
    }

    Collider2D[] Circlecast2D(Vector2 position, int mask, float min, float max)
    {
